...
def run_benchmark(server_url, config):
    ...
    if not is_sweep:
        return map_metrics(all_results[0])
...
def cmd_run(args):
    ...
    # Detect backend from serve.sh
    try:
        content = SERVE_SCRIPT.read_text().lower()
    except FileNotFoundError:
        print("ERROR: serve.sh not found")
        sys.exit(1)
    backend = args.backend or next((b for b in ("vllm", "sglang", "trtllm") if b in content), "unknown")
...
def cmd_remote(args):
    ...
    if args.remote_cmd == "sync":
        ...
        cmd = f"cd {remote_path} && uv run engine.py run{backend_flag} 2>&1"
        print(f"[remote] Running experiment on {host}...")
        result = _ssh_run(host, cmd, capture=True, timeout=args.timeout)
        print(result.stdout)
        if result.stderr:
