...
# HF token for gated Gemma access. Weights are already cached on yottah100
# (pre-fetched by `engine.py remote sync`), so this is only needed if the cache
# is invalidated. If unset, vLLM will still load from the local HF cache.
if [ -n "${HF_TOKEN:-}" ]; then
